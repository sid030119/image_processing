...
void main() {
    char InFileName[30] = "noise_8.bmp"; // .bmp 흑백이미지 받기
    char OutFileName[30] = "out_noise_prewitt.bmp"; // 노이즈 추가된 이미지
    FILE* infile;
    FILE* outfile;
    ...
    for (int i = 1; i < N - 1; i++) {
        for (int j = 1; j < N - 1; j++) {
            Gx = (input_image[i - 1][j - 1] + input_image[i][j - 1] + input_image[i + 1][j - 1] - input_image[i - 1][j + 1] - input_image[i][j + 1] - input_image[i + 1][j + 1]);
            Gy = (input_image[i - 1][j - 1] + input_image[i - 1][j] + input_image[i - 1][j + 1] - input_image[i + 1][j - 1] - input_image[i + 1][j] - input_image[i + 1][j + 1]);
